...
export default function Page() {
    ...
    try {
      ...
      const reviews = await Promise.all(
        reviewIds.map(async (id) => {
          const response = await fetch(`${urlApi}/reviews/getReview/${id}`);
          if (response.ok) {
  ...
  const follow = async () => {
    ...
    try {
      const response = await fetch(`${urlApi}/users/follow`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`${urlApi}/users/${userName.user.name}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
